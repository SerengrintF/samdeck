...
function renderMeta(): string {
  return pageShell(
    ...
    `
      ...
      <h2>4. 시즌 3 (S3)</h2>
      <p>
        S3 화면은 시즌 선택기에서 바로 고를 수 있습니다.
        장수·티어덱·공존·개척 데이터는 순차 등록하며, 올라오는 즉시 홈 순위와 조합 추천 목록에 반영됩니다.
        보유 장수와 「나의 조합」은 시즌별로 따로 저장되므로 S1·S2 목록과 섞이지 않습니다.
      </p>
      <p>
        데이터가 비어 있을 때는 빈 목록이 정상입니다.
        장수 목록을 받은 뒤에는 티어덱부터 채우고, 공존·개척은 그다음 단계에서 붙이면 됩니다.
      </p>
...
function renderPrivacy(): string {
  return pageShell(
    ...
    `
      ...
      <h3>평점·팁 API (설정된 경우)</h3>
      <p>
        이용자 평균 평점·조합 팁 기능을 사용할 때, 덱 식별자·점수·짧은 팁 본문·익명 투표 식별자 등이
        지정된 API 서버(예: api.samdeck.xyz)로 전송·집계될 수 있습니다.
        이름·이메일 등 직접 식별 정보를 평점·팁 목적으로 요구하지 않습니다.
      </p>
